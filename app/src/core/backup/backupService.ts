...
import type { ProjectRow, SettingRow, TaskRow, WorkspaceRow } from '../db/schema'
...
export async function buildBackup(): Promise<BackupFile> {
  const [workspace, settings, projects, tasks] = await Promise.all([
...
export async function importBackup(data: BackupFile): Promise<BackupSummary> {
  await db.transaction('rw', db.workspace, db.settings, db.projects, db.tasks, async () => {
